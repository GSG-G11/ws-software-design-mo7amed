...
document.querySelector('#form-unanswered').addEventListener('submit', function(e){
  viewData(e,'https://api.stackexchange.com/2.2/questions/unanswered?order=desc&sort=activity&site=stackoverflow&tagged=',"")
})
document.querySelector('#form-answerers').addEventListener('submit', function (e) {
  viewData(e,'http://api.stackexchange.com/2.2/tags/','/top-answerers/all_time?site=stackoverflow')
})

function viewData(e,url1,url2){
  e.preventDefault();
  const form = e.target;
  const tags = form.querySelector('input[name=tags]').value;
  const url  = url1 + tags + url2;
  requestAPI(viewUnanswered,url,tags)
}

function requestAPI(cb, url,tags){
  ...
  xhr.addEventListener('load', function () {
    ...
    document.querySelector('#results-body').innerHTML = response.items.map(cb).join('<br>')})
      
    xhr.open('GET', url);
    xhr.send();
}

function viewUnanswered(item){
  return `<div>
