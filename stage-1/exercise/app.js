...
const generalRequestFunction = function(e, url, cb) {
    e.preventDefault();
    ...
    xhr.addEventListener('load', function() {
        if (xhr.status === 200) {
            ...
            document.querySelector('#results-body').innerHTML = response.items.map(cb)
                .join('<br>');
...
document.querySelector('#form-unanswered').addEventListener('submit', function(e) {
    ...
    var url = 'https://api.stackexchange.com/2.2/questions/unanswered?order=desc&sort=activity&site=stackoverflow&tagged=' + e.target.querySelector('input[name=tags]').value;
    generalRequestFunction(e, url, function(item) {
        return '' +
            '<div>' +
...
document.querySelector('#form-answerers').addEventListener('submit', function(e) {
    var url = 'http://api.stackexchange.com/2.2/tags/' + e.target.querySelector('input[name=tags]').value + '/top-answerers/all_time?site=stackoverflow'
    generalRequestFunction(e, url, function(item) {
        return '' +
            '<div>' +
